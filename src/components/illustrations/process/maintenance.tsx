...
export function MaintenanceIllustration({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 120 120" className={className} xmlns="http://www.w3.org/2000/svg">
      ...
      {/* Central bolt */}
      <circle cx={60} cy={60} r={10} fill="none" className="stroke-steel" strokeWidth={1.5} />
      <circle cx={60} cy={60} r={4} fill="none" className="stroke-steel" strokeWidth={1.5} />
      {/* Bolt hex points */}
      {Array.from({ length: 6 }, (_, i) => {
        const angle = (i / 6) * Math.PI * 2;
        const x = 60 + Math.cos(angle) * 10;
        const y = 60 + Math.sin(angle) * 10;
        return <circle key={i} cx={x} cy={y} r={1.5} className="fill-steel" opacity={0.6} />;
      })}
      {/* Wrench */}
      <path d="M82 38 L 72 48" className="stroke-deep-blue" strokeWidth={2.5} strokeLinecap="round" />
      <path d="M82 38 L 90 30 Q 96 26 98 32 L 94 36 Z" fill="none" className="stroke-steel" strokeWidth={1.5} strokeLinejoin="round" />
