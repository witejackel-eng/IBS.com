    ...
    <section className="relative border-y border-border bg-secondary/40 bg-engineering-grid py-14">
      ...
      <Marquee>
        {partners.map((p, i) => (
          <span
            key={`${p.slug}-${i}`}
            className="shrink-0 cursor-pointer text-base font-semibold tracking-wide whitespace-nowrap text-tangerine-500 transition-[color,transform,filter] duration-[250ms] hover:-translate-y-0.5 hover:text-tangerine-600 hover:drop-shadow-[0_0_10px_rgba(249,115,22,0.35)] sm:text-xl lg:text-2xl font-heading"
          >
            {p.name}
          </span>
        ))}
